...
from src.data.downloader.binance_spot import BinanceSpotKlineClient, DATA_CONTRACT_COLUMNS
from src.data.downloader.models import DownloadPartitionResult, DownloadProgress, DownloadResult
from src.data.downloader.utils import (
    INTERVAL_MS,
    datetime_to_ms,
    format_utc_z,
    is_month_boundary,
...
def _download_partition(
    *,
    ...
    tmp_partition = partition_path(data_root / "tmp", exchange, symbol, timeframe, start_time)
    partition = partition_label(start_time)

    if _is_completed_partition_valid(raw_partition) and not force:
    ...
    try:
        tmp_partition.mkdir(parents=True, exist_ok=False)
        ...
        data_file = tmp_partition / "klines.parquet"
        ...
        write_json(tmp_partition / "metadata.json", metadata)

        if not data_file.exists() or data_file.stat().st_size == 0:
            raise OSError("tmp klines.parquet was not written")

        raw_partition.parent.mkdir(parents=True, exist_ok=True)
        _replace_raw_partition(tmp_partition, raw_partition)
        ...
        shutil.rmtree(tmp_partition, ignore_errors=True)
        if not raw_partition.exists():
            failed_metadata = {
                ...
                "failed_at": format_utc_z(datetime.now(UTC)),
                "schema_version": SCHEMA_VERSION,
                "downloader_version": DOWNLOADER_VERSION,
            }
            raw_partition.mkdir(parents=True, exist_ok=True)
            write_json(raw_partition / "metadata.json", failed_metadata)
...
def _is_completed_partition_valid(raw_partition: Path) -> bool:
    data_file = raw_partition / "klines.parquet"
    return (
...
def _replace_raw_partition(tmp_partition: Path, raw_partition: Path) -> None:
    if not raw_partition.exists():
        tmp_partition.rename(raw_partition)
        return
