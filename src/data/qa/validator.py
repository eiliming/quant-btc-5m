...
from src.data.downloader.utils import datetime_to_ms, format_utc_z, ms_to_datetime, write_json
from src.data.qa.models import PartitionSpec, QaStatus, RuleDefinition, RuleResult
...
RULES: tuple[RuleDefinition, ...] = (
    ...
    RuleDefinition("FILE_003", "Metadata Exists", "file", "WARNING"),
...
def validate_partition(
    ...
    *,
    ...
    report_root: str | Path = "data/qa_reports",
) -> dict[str, Any]:
    ...
    frame, read_error = _read_klines(partition_dir / "klines.parquet")
...
def run_all(
    *,
    ...
    raw_root = Path(root)
    qa_report_root = Path(report_root) if report_root is not None else raw_root.parent / "qa_reports"
        ...
        partition_reports.append(
            {
                ...
                "report_path": str(
                    qa_report_root / exchange / symbol / timeframe / year / month / "qa_report.json"
                ),
            }
        )

    summary = _summary_report(raw_root, partition_reports)
    write_json(qa_report_root / "summary_report.json", summary)
    return summary
...
def _file_002(partition_dir: Path) -> RuleResult:
    ...
    return _result(
        rule,
        valid,
        "klines.parquet exists and is non-empty."
        if valid
        else f"klines.parquet is missing or empty; size={size}.",
...
def _schema_001(frame: pd.DataFrame | None, read_error: str | None) -> RuleResult:
    ...
    missing = [column for column in DATA_CONTRACT_COLUMNS if column not in frame.columns]
    return _result(
        rule,
        not missing,
        "Required columns exist." if not missing else f"Missing required columns: {missing}.",
        expected=DATA_CONTRACT_COLUMNS,
...
def _schema_002(frame: pd.DataFrame | None, read_error: str | None) -> RuleResult:
    ...
    return _result(
        rule,
        actual == DATA_CONTRACT_COLUMNS,
        "Column order matches contract."
        if actual == DATA_CONTRACT_COLUMNS
...
def _schema_003(frame: pd.DataFrame | None, read_error: str | None) -> RuleResult:
    ...
    return _result(
        ...
        expected=_expected_dtypes(),
        actual={column: str(frame[column].dtype) for column in DATA_CONTRACT_COLUMNS},
    )
...
def _read_klines(path: Path) -> tuple[pd.DataFrame | None, str | None]:
    if not path.is_file() or path.stat().st_size == 0:
        return None, "klines.parquet is missing or empty"
    try:
        return pd.read_parquet(path), None
    except Exception as exc:
        return None, f"failed to read klines.parquet: {exc}"
...
def _unreadable_result(
    ...
    *,
    ...
    return _result(
        rule,
        False,
        f"Cannot validate rule because klines.parquet is unreadable: {read_error}.",
        expected=expected,
        actual=None,
    )
