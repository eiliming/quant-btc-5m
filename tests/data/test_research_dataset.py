...
from src.data.downloader.utils import datetime_to_ms, write_json
from src.data.research_dataset.builder import build_dataset
from src.data.research_dataset.cli import main as research_cli_main
from src.data.research_dataset.loader import load_dataset
from src.data.research_dataset.schema import OHLCV_COLUMNS, RESEARCH_COLUMNS, SCHEMA_VERSION, timeframe_to_ms
...
def qa_report_path(root: Path, year: int, month: int, *, timeframe: str = "5m") -> Path:
    return root / "qa_reports" / "binance_spot" / "BTCUSDT" / timeframe / f"{year:04d}" / f"{month:02d}" / "qa_report.json"
...
def write_partition(
    ...
    *,
    ...
    if qa_status is not None:
        write_json(
            qa_report_path(root, year, month, timeframe=timeframe),
            {"report_type": "partition_qa", "schema_version": "v1", "status": qa_status},
        )
...
class ResearchDatasetTests(unittest.TestCase):
    def test_build_dataset_writes_schema_metadata_and_drops_is_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            metadata = build_dataset(
                ...
                "5m",
                raw_root=root / "raw",
                qa_report_root=root / "qa_reports",
                output_root=root / "research",
            )

            dataset_path = root / "research" / "binance_spot" / "BTCUSDT" / "5m" / "dataset.parquet"
            metadata_path = root / "research" / "binance_spot" / "BTCUSDT" / "5m" / "metadata.json"
            ...
            self.assertEqual(written_metadata["row_count"], len(dataset))
            self.assertEqual(written_metadata["start_time_utc"], "2024-01-01T00:00:00Z")
            self.assertEqual(written_metadata["end_time_utc"], "2024-02-29T23:55:00Z")
            self.assertTrue(written_metadata["created_at_utc"].endswith("Z"))
    ...
    def test_build_dataset_fails_without_passed_qa(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write_json(qa_report_path(root, 2024, 1), {"qa_status": "FAIL"})
    ...
    def test_cli_build_and_inspect_render_json(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(json.loads(build_stdout.getvalue())["schema_version"], SCHEMA_VERSION)
