...
from src.ingestion.downloader.binance_spot import DATA_CONTRACT_COLUMNS, convert_klines_to_dataframe
...
def write_completed_partition(partition_dir: Path, frame: pd.DataFrame | None = None) -> None:
    ...
    active_frame = frame if frame is not None else contract_frame()
    active_frame.to_parquet(partition_dir / "data.parquet", index=False)
    write_json(
...
class DownloaderTests(unittest.TestCase):
    ...
    def test_staging_partition_is_moved_to_raw_after_success(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            data_root = Path(temp_dir)
            client = FakeClient(contract_frame())
            ...
            self.assertFalse(staging_partition.exists())

            written = pd.read_parquet(partition_dir / "data.parquet")
    ...
    def test_empty_download_does_not_create_partial_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            data_root = Path(temp_dir)
            ...
            return DownloadResult(
                ...
                partitions=[
                    DownloadPartitionResult(
                        ...
                        end_time=end_time,
                        status="skipped",
                        path="artifacts/raw/binance_spot/BTCUSDT/5m/2024/01",
                    )
                ],
            )
